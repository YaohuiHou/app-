<template>
	<div class="load-data">
		<!-- 加载完成 -->
	  <div class="completed" v-show="completedShow">我是有底线的</div>
	  <!-- 正在加载 -->
	  <div class="loading" v-show="loading">
	  	<span class="loading-ring">等</span>
	  </div>
	  <!-- 暂无帮助 -->
	  <div class="no-help" v-if="nothing">
	  	<span>无</span>
	  	<p>暂无帮助，快来抢沙发呦~</p>
	  </div>
	</div>
...
<script>
	export default {	
		props:['completedShow','loading','nothing'],
	  data () {
	    return {
	    }
	  },
...
<style scoped lang="less">
    ...
    &:before{
	    ...
	    width: 100%;
	    height: 30px;
	    background: -webkit-gradient(linear, 0 0, 100% 0,from(#f5f5f5), color-stop(0.1, rgba(245,245,245,0)), color-stop(0.9, rgba(245,245,245,0)),to(#f5f5f5));
